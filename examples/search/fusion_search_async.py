"""
Example demonstrating asynchronous fusion search with the Fusionbase SDK.

This example shows how to search across all entity types with a single query asynchronously.
"""
...
async def display_entity_result(entity_type, entity_data):
...
async def run_fusion_search():
    """Run asynchronous fusion searches."""
    print("=== Asynchronous Fusion Search Example ===\n")
    ...
    try:
        # Example 1: Basic async fusion search
        query = "health insurance"
        print(f"\n1. Basic async fusion search for '{query}'...")
        ...
        for entity_type, entities in results.results.items():
            if entities:
                ...
                # Show top result for each type
                if entities:
                    print("\nTop result:")
                    await display_entity_result(entity_type, entities[0])

        print(f"\nTotal results across all entity types: {total_count}")

        # Example 2: Multiple concurrent searches
        print("\n2. Running multiple async fusion searches concurrently")

        # Create queries for different searches
        queries = [
            "renewable energy", "artificial intelligence", "climate change"
        ]
        search_params = [FusionSearchParams(q=query) for query in queries]

        # Run searches concurrently
        search_tasks = [
            client.search.fusion.asearch(params) for params in search_params
        ]
        search_results = await asyncio.gather(*search_tasks)

        # Display results for all searches
        for i, (query, result) in enumerate(zip(queries, search_results), 1):
            print(f"\nSearch {i} for '{query}':")

            # Count results in each category
            counts = {
                entity_type: len(entities)
                for entity_type, entities in result.results.items()
                if entities
            }
            ...
            for entity_type, count in counts.items():
                print(f"    {entity_type.upper()}: {count} results")

            # Show top rank
            if result.ranks:
                print(f"  Top ranked entity type: {result.ranks[0]}")
