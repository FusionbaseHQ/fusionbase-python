...
class TestPrepareSearchParams(unittest.TestCase):
    ...
    def test_prepare_with_kwargs(self):
        ...
        params, query_dict = prepare_search_params(
...
class TestProcessSearchResults(unittest.TestCase):
    ...
    def test_process_results_creates_lazy_references(self):
        ...
        response_data = {
            "results": [
                {"entity": {"fb_entity_id": "person_123"}},
                {"entity": {"fb_entity_id": "person_456"}},
            ],
            "total": 2
        }
    ...
    def test_process_skips_invalid_results(self):
        ...
        response_data = {
            "results": [
                {"entity": {"fb_entity_id": "valid_1"}},
                {"not_entity": "invalid"},  # Missing 'entity' key
                "not_a_dict",  # Not a dict
                {"entity": {}},  # Missing fb_entity_id
                {"entity": {"fb_entity_id": "valid_2"}},
            ],
            "total": 5
        }
...
class TestMakeSearchRequest(unittest.TestCase):
    ...
    def test_make_request_with_request_method(self):
        ...
        mock_client.request.assert_called_once_with(
            "GET", "search/persons", params={"q": "test"}
        )
...
async def test_make_search_request_async_with_arequest():
    ...
    result = await make_search_request_async(
        client=mock_client,
        endpoint="search/persons",
        params={"q": "async test"}
    )

    mock_client.arequest.assert_called_once_with(
        "GET", "search/persons", params={"q": "async test"}
    )
...
async def test_make_search_request_async_raises_when_no_method():
    ...
    with pytest.raises(APIError) as exc_info:
        await make_search_request_async(
            client=mock_client,
            endpoint="search/persons",
            params={"q": "test"}
        )
...
class TestSearchUtilsIntegration(unittest.TestCase):
    ...
    def test_full_search_flow(self):
        ...
        mock_client.request.return_value = {
            "results": [
                {"entity": {"fb_entity_id": "person_1"}},
                {"entity": {"fb_entity_id": "person_2"}},
            ],
            "total": 2
        }
