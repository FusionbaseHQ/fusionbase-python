...
class TestFeature(unittest.TestCase):
    ...
    def test_feature_from_id_mock(self):
        ...
        with patch('fusionbase.utils.api_utils.make_entity_request') as mock_request:
            ...
            mock_request.assert_called_once_with(
                mock_client, "feature", "feature_abc"
            )
...
async def test_feature_afrom_id_mock():
    ...
    with patch('fusionbase.utils.api_utils.make_entity_request_async') as mock_request:
        mock_request.return_value = mock_response_data
        ...
        mock_request.assert_called_once_with(
            mock_client, "feature", "feature_async"
        )
...
class TestFeatureComplexValues(unittest.TestCase):
    ...
    def test_feature_nested_dict_value(self):
        ...
        feature = Feature(
            fb_entity_id="complex_id",
            fb_entity_version="v1",
            value=complex_value
        )
        self.assertEqual(
            feature.value["level1"]["level2"]["level3"],
            "deep_value"
        )
